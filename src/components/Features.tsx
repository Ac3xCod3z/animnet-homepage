  ...
  return (
    <section id="features" className="py-20 glass">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl md:text-4xl font-bold text-center mb-12 manga-text">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <div key={index} className="p-6 rounded-lg glass hover:bg-white/10 transition-colors manga-border">
              <feature.icon className="w-12 h-12 mb-4 text-blue-400" />
              <h3 className="text-xl font-semibold mb-2 manga-text">{feature.title}</h3>
              <p className="text-gray-400 manga-text">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
